...
import { getMiDashboard } from '../services/comercioService'
import type { DashboardComercioResponse } from '../services/comercioService'
...
export default function DashboardComercioPage() {
  ...
  const [dashboard, setDashboard] = useState<DashboardComercioResponse | null>(null)
  ...
  useEffect(() => {
    getMiDashboard()
      .then(data => {
        setDashboard(data)
        // Mapear lotes reales al formato de la tabla
        setLotes(data.lotesRecientes.map(l => ({
          id: l.id.substring(0, 12).toUpperCase(),
          nombre: l.nombre,
          cantidad: l.cantidad,
          hace: l.hace,
          estado: l.estado.toLowerCase(),
          img: l.fotoUrl || 'https://images.unsplash.com/photo-1619566636858-adf3ef46400b?w=80&q=70',
        })))
      })
      .catch(() => {})
        ...
        <section className={styles.hero}>
          <div className={styles.heroLeft}>
            ...
            <div className={styles.heroMonto}>
              ${Math.floor(dashboard?.ingresosMesActual ?? 0)}
              <span>.{String(Math.round(((dashboard?.ingresosMesActual ?? 0) % 1) * 100)).padStart(2, '0')}</span>
            </div>
            <p className={styles.heroSub}>
              {(dashboard?.porcentajeCambioIngresos ?? 0) >= 0 ? 'Aumento' : 'Reducción'} sostenido ·{' '}
              <strong>{(dashboard?.porcentajeCambioIngresos ?? 0) >= 0 ? '+' : ''}{dashboard?.porcentajeCambioIngresos ?? 0}%</strong> vs mes anterior
            </p>
          </div>
          <div className={styles.heroStats}>
            <div className={styles.heroStat}><span className={styles.heroStatNum}>{(dashboard?.totalKgRescatados ?? 0).toLocaleString()}</span><span className={styles.heroStatLabel}>kg rescatados</span></div>
            <div className={styles.heroStatDiv}/>
            <div className={styles.heroStat}><span className={styles.heroStatNum}>{dashboard?.reservasPendientes ?? 0}</span><span className={styles.heroStatLabel}>reservas pendientes</span></div>
            <div className={styles.heroStatDiv}/>
            <div className={styles.heroStat}><span className={styles.heroStatNum}>{(dashboard?.personasBeneficiadasMes ?? 0).toLocaleString()}</span><span className={styles.heroStatLabel}>personas beneficiadas</span></div>
          </div>
        </section>
        ...
        <div className={styles.body}>
          ...
          <div className={styles.colMain}>
            ...
            <div className={styles.statsRow}>
              {[
                { label: 'Lotes activos hoy', val: String(dashboard?.lotesActivos ?? 0), icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/></svg>, color: styles.iconVerde },
                { label: 'Reservas pendientes', val: String(dashboard?.reservasPendientes ?? 0), icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>, color: styles.iconDorado },
                { label: 'CO₂ evitado (kg)', val: String(dashboard?.co2EvitadoKg ?? 0), icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 20A7 7 0 0 1 9.8 6.1C15.5 5 17 4.48 19 2c1 2 2 4.18 2 8 0 5.5-4.78 10-10 10Z"/></svg>, color: styles.iconVerde },
                { label: 'Beneficiarios este mes', val: String(dashboard?.personasBeneficiadasMes ?? 0), icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>, color: styles.iconCoral },
              ].map((s, i) => (
                  ...
                  <tbody>
                    {lotes.map(l => (
                      <tr key={l.id} className={styles.tableRow}>
                        ...
                        <td>{l.hace}</td>
                        <td>
                          <span className={`${styles.badge} ${(ESTADO_CFG[l.estado.toLowerCase()] ?? ESTADO_CFG['activo']).cls}`}>
                            {(ESTADO_CFG[l.estado.toLowerCase()] ?? ESTADO_CFG['activo']).label}
                          </span>
                        </td>
          ...
          <div className={styles.colSide}>
            <div className={styles.heroMesCard}>
              ...
              <h3 className={styles.heroMesTitulo}>{dashboard?.tituloLogro ?? 'Héroe del Mes'}</h3>
              <p className={styles.heroMesSub}>{dashboard?.descripcionLogro ?? 'Sigue rescatando alimentos para tu comunidad.'}</p>
            ...
            <div className={styles.urgentesCard}>
              <div className={styles.urgentesHeader}>
                ...
                <span className={styles.urgentesBadge}>{dashboard?.reservasUrgentes?.length ?? 0} hoy</span>
              </div>
              {(dashboard?.reservasUrgentes ?? []).length === 0 ? (
                <p style={{ fontSize: '13px', color: '#667A70', padding: '8px 0' }}>
                  No hay reservas urgentes pendientes.
                </p>
              ) : (dashboard?.reservasUrgentes ?? []).map((r, i) => (
                <div key={i} className={styles.urgenteItem}>
                  <div className={styles.urgenteAvatar}>{r.organizacion.charAt(0)}</div>
                  <div className={styles.urgenteInfo}>
                    <p className={styles.urgenteOrg}>{r.organizacion}</p>
                    <p className={styles.urgenteProd}>{r.producto} · {r.hora}</p>
                  </div>
